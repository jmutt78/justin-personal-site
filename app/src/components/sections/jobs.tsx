...
const StyledHighlight = styled.div<{ activeTabId: number | string | undefined | null }>`
  position: absolute;
  top: 0;
  left: 0;
  z-index: 10;
...
const Jobs = () => {
  ...
  const [activeTabId, setActiveTabId] = useState<number | string | undefined | null>(null);
  ...
  const focusTab = () => {
    if (tabs?.current[tabFocus ?? 0]) {
      (tabs.current[tabFocus ?? 0] as HTMLButtonElement).focus();
      return;
    }
  ...
  return (
    <StyledJobsSection id='jobs' ref={revealContainer as RefObject<HTMLDivElement>}>
